...
def train_evolutionary(args):
    ...
    if args.live_plot is not None:
        ...
        ax_seq_viz = fig.add_subplot(gs[1, 2])
        ...
        ax_seq_viz.set_title('Best Sequence Patterns', fontweight='bold')
        ax_seq_viz.axis('off')
    ...
    for generation in tqdm(range(args.generations), desc="Evolution"):
        ...
        if args.live_plot is not None and (generation % args.live_plot == 0 or generation == args.generations - 1):
            ...
            # Best sequence pattern visualization
            ax_seq_viz.clear()
            ax_seq_viz.set_title('Best Sequence Patterns', fontweight='bold')
            ax_seq_viz.axis('off')
            
            # Create a visual representation of write patterns in the sequence
            write_actions = [a for a in optimizer.best_sequence if a >= 5]
            if write_actions:
                n_patterns = len(write_actions)
                cols = min(5, n_patterns)
                rows = (n_patterns + cols - 1) // cols
                
                for idx, action in enumerate(write_actions):
                    pattern_index = action - 5
                    ...
                    # Position in the subplot (normalized coordinates)
                    x_pos = col / cols + 0.1 / cols
                    y_pos = 1 - (row / rows) - 0.1 / rows
                    width = 0.8 / cols
                    height = 0.8 / rows
                    
                    # Create mini axes for each pattern
                    mini_ax = ax_seq_viz.inset_axes([x_pos, y_pos - height, width, height])
                    mini_ax.imshow(pattern, cmap='binary', vmin=0, vmax=1, interpolation='nearest')
                    mini_ax.set_title(f'{action_labels[action]}', fontsize=8)
                    mini_ax.set_xticks([])
                    mini_ax.set_yticks([])
            else:
                ax_seq_viz.text(0.5, 0.5, 'No write patterns\nin sequence', 
