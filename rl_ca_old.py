...
#             actor_loss, critic_loss = agent.train_step(state, np.array([action]), np.array([reward], dtype=np.float32), next_state, np.array([done]))

#             state = next_state
#             episode_reward += reward
#             # The loss is a scalar tensor. Call .numpy() directly to get the number.
...
#         if (episode + 1) % 25 == 0:
#             agent.model.save_weights(f'ca_agent_weights_{episode+1}.h5')
#             print(f"\nSaved weights at episode {episode+1}")

#     agent.model.save_weights('ca_agent_weights_final.h5')
#     print("\n--- Training Complete ---")
#     print(f"Final weights saved to ca_agent_weights_final.h5")
...
def run_demo(args):
    ...
    title = ax.set_title("Step: 0 | Mode: " + ("Manual" if manual_mode else "Agent"))
    plt.xticks([])
    plt.yticks([])

    global current_action
    current_action = 4 # Default to 'do_nothing'

    def update(frame):
        global current_action
        if manual_mode:
            action = current_action
            current_action = 4 # Reset to do_nothing after each key press
        else:
            action = agent.select_action(state)
        ...
        title.set_text(f"Step: {frame} | Last Action: {env.actions[action]}")
        
        return grid_img, agent_patch, target_patch, title
    ...
    if manual_mode:
        ...
        print("\nManual Control Enabled:\n- W/A/S/D or Arrow Keys to move\n- Spacebar to pass (do nothing)\n- Focus the plot window to register keys.")

    ani = animation.FuncAnimation(fig, update, frames=args.steps, blit=True, interval=100, repeat=False)
    plt.show()
