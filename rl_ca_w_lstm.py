...
class PPOAgentLSTM:
    """
    PPO agent with LSTM for sequence learning.
    The LSTM maintains hidden state across timesteps to learn action sequences.
    """
    def __init__(self, state_shape, num_actions, learning_rate=0.0003, gamma=0.99, 
                 lam=0.95, clip_eps=0.1, entropy_coef=0.01, vf_coef=0.5, lstm_units=256):
    ...
    def _build_networks(self):
        # Input for spatial state
        input_layer = layers.Input(shape=self.state_shape, name='state_input')
        
        # CNN feature extractor (spatial processing) - NO compression
        x = ToroidalPadding(name='toroidal_pad')(input_layer)
        x = layers.Conv2D(32, 3, activation='relu', padding='valid', name='conv1')(x)
        ...
        # Flatten the entire spatial feature map (NO dense compression layer)
        # This preserves all spatial information for the LSTM
        features = layers.Flatten(name='flatten')(x)
        
        # Get the feature dimension
        feature_dim = features.shape[-1]
        print(f"CNN feature dimension (full spatial): {feature_dim}")
        
        # Reshape for LSTM: (batch, timesteps, features)
        # We'll use timesteps=1 for online learning, but LSTM maintains state
        features_seq = layers.Reshape((1, feature_dim), name='reshape')(features)
        
        # LSTM layer for sequence learning
        # stateful=False because we'll manage state manually for flexibility
        lstm_out = layers.LSTM(self.lstm_units, return_sequences=True, name='lstm')(features_seq)
        lstm_flat = layers.Flatten(name='lstm_flatten')(lstm_out)
        
        # Actor head (policy)
        action_logits = layers.Dense(self.num_actions, name='policy_output')(lstm_flat)
        
        # Critic head (value function)
        state_value = layers.Dense(1, name='value_output')(lstm_flat)

        self.model = keras.Model(inputs=input_layer, outputs=[action_logits, state_value])
        
        # Store references to layers for manual forward pass
        self.toroidal_pad = self.model.get_layer('toroidal_pad')
        self.conv1 = self.model.get_layer('conv1')
        self.conv2 = self.model.get_layer('conv2')
        self.conv3 = self.model.get_layer('conv3')
        self.flatten_layer = self.model.get_layer('flatten')
        self.reshape_layer = self.model.get_layer('reshape')
        self.lstm_layer = self.model.get_layer('lstm')
        self.lstm_flatten = self.model.get_layer('lstm_flatten')
        self.policy_layer = self.model.get_layer('policy_output')
        self.value_layer = self.model.get_layer('value_output')
    
    def reset_state(self):
        """Reset LSTM hidden state (call at episode start)"""
        self.h_state = np.zeros((1, self.lstm_units), dtype=np.float32)
        self.c_state = np.zeros((1, self.lstm_units), dtype=np.float32)
    
    def select_action(self, state, training=True):
        """Select action with LSTM state management"""
        if training:
            # Use current LSTM state
            logits, _ = self._forward_with_state(state)
    ...
    def _forward_with_state(self, state):
        """Forward pass maintaining LSTM state"""
        # Manually apply each layer in sequence
        x = self.toroidal_pad(state)
        x = self.conv1(x)
        x = self.conv2(x)
        x = self.conv3(x)
        ...
        # Apply LSTM with state tracking
        # LSTM returns a tuple: (output, final_state) where final_state is [h, c]
        lstm_result = self.lstm_layer(
            x, 
            initial_state=[self.h_state, self.c_state]
        )
        
        # Unpack the result correctly
        if isinstance(lstm_result, tuple):
            lstm_out, states = lstm_result
            self.h_state, self.c_state = states
        else:
            # If no states returned (shouldn't happen with initial_state)
            lstm_out = lstm_result
        
        # Flatten and get outputs
        x_flat = self.lstm_flatten(lstm_out)
        logits = self.policy_layer(x_flat)
        value = self.value_layer(x_flat)
        
        return logits, value

    def get_action_and_value(self, state):
        """Get action, log prob, and value with LSTM state"""
        logits, value = self._forward_with_state(state)
    ...
    def get_action_probs_and_value(self, state):
        """Get action probabilities and value for visualization"""
        logits, value = self._forward_with_state(state)
    ...
    def compute_gae(self, rewards, values, dones, last_value):
        """Compute Generalized Advantage Estimation (same as before)"""
        advantages = []
        gae = 0.0
    ...
    def ppo_update(self, states, actions, old_log_probs, returns, advantages):
        """
        PPO update for LSTM - processes full sequences
        Note: For LSTM training, we should ideally process sequences,
        but for simplicity we'll do per-step updates with state reset
        """
        with tf.GradientTape() as tape:
            # Forward pass without state management (training on collected data)
...
def train_agent_lstm(args):
    """Main PPO training loop with LSTM rollout collection and minibatch updates."""
    print("--- Starting PPO Training with LSTM ---")
    ...
    agent = PPOAgentLSTM(
        ...
        lstm_units=getattr(args, 'lstm_units', 256)
    )
    ...
    if args.live_plot is not None:
        ...
        fig.suptitle('PPO Training Progress (Live) [LSTM]', fontsize=16, fontweight='bold')
    ...
    for episode in range(args.episodes):
        # CRITICAL: Reset LSTM state at episode start
        agent.reset_state()
        ...
        for step in pbar:
            # LSTM maintains state across steps within episode
        ...
        if (episode + 1) % 25 == 0:
            agent.model.save_weights(f'ca_agent_lstm_weights_{episode+1}.weights.h5')
            print(f"\nSaved LSTM weights at episode {episode+1}")

    # Save final weights
    agent.model.save_weights('ca_agent_lstm_weights_final.weights.h5')
    ...
    with open('training_metrics_lstm.json', 'w') as f:
        json.dump(metrics_data, f)
    print("Training metrics saved to training_metrics_lstm.json")
    ...
    plt.tight_layout()
    plt.savefig('ppo_training_results_lstm.png', dpi=150, bbox_inches='tight')
    plt.close('all')
    print("Final plot saved to ppo_training_results_lstm.png")
...
if __name__ == '__main__':
    ...
    train_parser.add_argument('--clip-eps', type=float, default=0.3, help='PPO clip epsilon.')
    ...
    train_parser.add_argument('--ppo-epochs', type=int, default=8, help='PPO update epochs per rollout.')
    ...
    train_parser.add_argument('--lstm-units', type=int, default=256, help='LSTM unit size.')
